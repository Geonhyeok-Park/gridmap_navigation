...
public:
  ...
  void transform(const typename pcl::PointCloud<pointType>::Ptr &cloudIn,
  ...
  void passthroughFilter(typename pcl::PointCloud<pointType>::Ptr &cloud,
                         ...
                         double minZ,
                         double maxZ,
                         double filterAngle = 0.0);
  void passthroughFilter(typename pcl::PointCloud<pointType>::Ptr &cloud,
                         const std::string &fieldName,
                         double minVal,
                         double maxVal);

  void voxelFilter(typename pcl::PointCloud<pointType>::Ptr &cloud,
                   float lx,
                   float ly,
                   float lz);

  void computeNormals(typename pcl::PointCloud<pointType>::Ptr &cloudIn,
  ...
  void pubNormalArrow(
      const pcl::PointXYZINormal &normal,
      ros::Publisher &publisher,
      const std::string &frameId,
      const ros::Time timestamp,
      const std::string &RGB);

  void pubNormalArrows(
...
template <typename pointType>
void CloudManager<pointType>::transform(const typename pcl::PointCloud<pointType>::Ptr &cloudIn,
                                        typename pcl::PointCloud<pointType>::Ptr &cloudOut,
                                        const geometry_msgs::Transform &tf,
                                        const std::string targetFrame)
{
  if (cloudIn->header.frame_id.empty())
    ROS_ERROR("PCL cloud has no frame_id. Check the code inside.");

  if (cloudIn->empty())
    ROS_ERROR("point cloud empty. Skip transform of the point cloud");
...
template <typename pointType>
void CloudManager<pointType>::publish(const typename pcl::PointCloud<pointType>::Ptr &msg,
                                      const ros::Publisher &publisher)
{
...
template <typename pointType>
void CloudManager<pointType>::pubNormalArrow(
    const pcl::PointXYZINormal &normal,
    ros::Publisher &publisher,
    const std::string &frameId,
    const ros::Time timestamp,
    const std::string &rgb)
{
  ...
  // marker.scale.y = 0.0; // haed diameter
  // marker.scale.z = 0.0; // head length
  // float shaft_length = 10;
